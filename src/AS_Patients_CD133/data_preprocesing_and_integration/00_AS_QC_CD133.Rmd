...
Doublet_rate_predict <- function(cell_loaded_10x, verbose = FALSE) {
  ...
  # More info: https://kb.10xgenomics.com/hc/en-us/articles/360001378811-What-is-the-maximum-number-of-cells-that-can-be-profiled-
  # Created 15/02/2021
  
  multiplet_rate <- c(0.004, 0.008, 0.016, 0.023, 0.031,
                      0.039, 0.046, 0.054, 0.061, 0.069, 0.076)
  
  cell_loaded <- c(800, 1600, 3200, 4800, 6400, 
                   8000, 9600, 11200, 12800, 14400, 16000)
  
  multiplet_df <- data.frame(multiplet_rate = multiplet_rate, 
...
## Count matrices procesed by Cell Ranger
input_folder <- config$raw_data$dir

## QC plots, QC csv
output_folder_qc <- config$filtered_data$qc_dir
dir.create(output_folder_qc, recursive = TRUE, showWarnings = FALSE)

## Count matrices processed and filtered
output_folder <- config$filtered_data$dir
dir.create(output_folder, recursive = TRUE, showWarnings = FALSE)

# List of files to load
AS_files <- list.files(path = input_folder, full.names = TRUE)
AS_files <- AS_files[grep(pattern = "CD133|cd133", x = AS_files)]


sc_list <- vector(mode = "list", length = length(AS_files))
...
stats_cells_qc <- data.frame(n_cells_10x = "", n_cells_before_doublets = "", 
                             ...
                             mitoratio_mean_pre = "", mitoratio_median_pre = "",
                             mitoratio_sd_pre = "", gene_mean_pre = "", 
                             gene_median_pre = "", gene_sd_pre = "", 
                             counts_mean_pre = "", counts_median_pre = "", 
                             counts_sd_pre = "", mitoratio_mean_filt = "", 
                             mitoratio_median_filt = "", mitoratio_sd_filt = "", 
                             gene_mean_filt = "", gene_median_filt = "", 
                             gene_sd_filt = "", counts_mean_filt = "", 
                             counts_median_filt = "", counts_sd_filt = "")
for (sc_file in 1:length(AS_files)) {
  set.seed(47)
  if (file.exists(file.path(output_folder,paste0(
                         gsub(pattern = ".sc_file", replacement = "",
                              basename(AS_files[[sc_file]])),
                         config$filtered_data$sc_file_pattern)))) {
    
    print(paste0("Exist ", basename(AS_files[[sc_file]])))
    next
  }
  else {
    ...
    dir.create(
      recursive = TRUE, 
      path = file.path(output_folder_qc,
                    config$qc_parameters$qc_plots_dir,paste0(
                    gsub(pattern = ".sc_file",
                         replacement = "",
                         basename(AS_files[[sc_file]])))))

  print(AS_files[[sc_file]])
  sc_obj <- CreateSeuratObject(
    counts = Read10X(
      data.dir = file.path(AS_files[[sc_file]], "outs","filtered_feature_bc_matrix")),
    project = basename(AS_files[[sc_file]]))
  
  
  ncell_pre <- ncol(sc_obj)
  sc_obj$orig.ident <- rep(gsub(pattern = ".sc_file", replacement = "",
                                x = basename(AS_files[[sc_file]])), 
                           length(sc_obj$orig.ident))
  
  ncells <- length(sc_obj$orig.ident)
  
  sc_obj[["mitoRatio"]] <- PercentageFeatureSet(sc_obj, pattern = "^MT-")
  Seurat::VlnPlot(object = sc_obj, 
                  features = colnames(sc_obj@meta.data)[c(2:4)],
                  pt.size = 0.1, ncol = 3)
  ggsave(
    filename = file.path(output_folder_qc,config$qc_parameters$qc_plots_dir,
                      gsub(pattern = ".sc_file", replacement = "",
                           basename(AS_files[[sc_file]])),
                      "/vln_pre.pdf"), width = 6, device = "pdf", dpi = 320)

  sc_obj <- Seurat::NormalizeData(
    object = sc_obj, normalization.method = "LogNormalize", scale.factor = 10000)
  sc_obj <- Seurat::FindVariableFeatures(
    object = sc_obj, selection.method = "vst", nfeatures = 2000)
  sc_obj <- Seurat::ScaleData(
    object = sc_obj, features = Seurat::VariableFeatures(object = sc_obj))
  sc_obj <- Seurat::RunPCA(
    object = sc_obj, features = Seurat::VariableFeatures(object = sc_obj))
  sc_obj <- Seurat::RunUMAP(
    sc_obj, dims = 1:10)
  umap1 <- FeaturePlot(
    object = sc_obj, features = c("nCount_RNA"),min.cutoff = "q5", 
    max.cutoff = "q90") 
  umap2 <- FeaturePlot(
    object = sc_obj, features = c("nFeature_RNA"),min.cutoff = "q5", 
    max.cutoff = "q90") 
  umap3 <- FeaturePlot(
    object = sc_obj, features = c("mitoRatio"), min.cutoff = "q5",
    max.cutoff = "q90") 

  ## Filter criteria
  pass_mito_AND_nfeatures <- (sc_obj$nFeature_RNA >= config$qc_parameters$nfeature_rna_filt) & 
    (sc_obj$mitoRatio < config$qc_parameter$mito_ratio_filt)
  ...
  ggarrange_pre <- annotate_figure(
    ggarrange_pre, top = paste0("QC_pre ",
                                gsub(pattern = ".sc_file", 
                                     replacement = "",
                                     basename(AS_files[[sc_file]]))))
  
  ggsave(
    plot = ggarrange_pre, filename = file.path(output_folder_qc,
                                            config$qc_parameters$qc_plots_dir,
                                            gsub(pattern = ".sc_file", 
                                                 replacement = "",
                                                 basename(AS_files[[sc_file]])),
                                            "/UMAP_pre.pdf"), width = 6, 
    device = "pdf", dpi = 32, bg = "white")
  ...
  ggplot(data = bcmvn_sc, aes(x = pK, y = BCmetric, group = 1)) +
    ...
    geom_vline(xintercept = 
                 as.integer(bcmvn_sc[max(bcmvn_sc$BCmetric,
                                         na.rm = TRUE) == bcmvn_sc$BCmetric,2]),
               color = "red")
  ...
  print(
    as.numeric(
      as.character(
        bcmvn_sc[max(bcmvn_sc$BCmetric, 
                     na.rm = TRUE) == bcmvn_sc$BCmetric,2][1])))
  sc_obj <- doubletFinder_v3(
    sc_obj, PCs = 1:10, pN = 0.25, 
    pK = as.numeric(
      as.character(bcmvn_sc[max(bcmvn_sc$BCmetric, 
                                na.rm = TRUE) == bcmvn_sc$BCmetric,2][1])), 
    nExp = nExp_poi,reuse.pANN = FALSE, sct = FALSE)
  
  ## scds
  scds_result <- scds::cxds_bcds_hybrid(
    sce = as.SingleCellExperiment(sc_obj), estNdbl = FALSE)
  sc_obj <- AddMetaData(
    sc_obj, metadata = scds_result$hybrid_score, col.name = "hybrid_score")
  sc_obj <- AddMetaData(
    sc_obj ,metadata = sc_obj$hybrid_score >= 
  ...
  true_names_df_scds <- 
    true_names_doubletFinder[which(true_names_doubletFinder %in% 
                                     true_names_scds)]
  ...
  sc_obj <- Seurat::NormalizeData(
    object = sc_obj, normalization.method = "LogNormalize", 
    scale.factor = 10000)
  sc_obj <- Seurat::FindVariableFeatures(
    object = sc_obj, selection.method = "vst", nfeatures = 2000)
  sc_obj <- Seurat::ScaleData(object = 
                                
                                sc_obj, 
                              features = VariableFeatures(object = sc_obj))
  sc_obj <- Seurat::RunPCA(object = 
                           sc_obj, features = VariableFeatures(object = sc_obj))
  sc_obj <- Seurat::RunUMAP(sc_obj, dims = 1:10)
  
  umap1_post <- FeaturePlot(object = 
                              sc_obj, features = c("nCount_RNA"),
                            min.cutoff = "q5", max.cutoff = "q90") 
  umap2_post <- FeaturePlot(object = 
                              sc_obj, features = c("nFeature_RNA"),
                            min.cutoff = "q5", max.cutoff = "q90") 
  umap3_post <- FeaturePlot(object = 
                              sc_obj, features = c("mitoRatio"), 
                            min.cutoff = "q5", max.cutoff = "q90") 
  
  ggarrange_post <- ggarrange(umap1_post,umap2_post,umap3_post, 
                              nrow = 2,ncol = 2) 
  
  ggarrange_post <- annotate_figure(ggarrange_post, 
                                    top = paste0("QC_pre ",
                                                 gsub(pattern = ".sc_file", 
                                                      replacement = "",
                                                basename(AS_files[[sc_file]]))))
  ...
  stats_cells_qc[basename(AS_files[sc_file]),] <- stats_vector
  ...
  SeuratDisk::SaveH5Seurat(
    ...
    filename = file.path(output_folder,
                         paste0(gsub(pattern = ".sc_file", replacement = "",
                                     basename(AS_files[[sc_file]])),
                                config$filtered_data$sc_file_pattern)))
  gc()
...
write.csv(
  x = stats_cells_qc, 
  file = file.path(output_folder_qc,config$qc_parameters$qc_stats_file))
```
